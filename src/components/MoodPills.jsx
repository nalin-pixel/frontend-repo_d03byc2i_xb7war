    ...
    <section className="py-12 sm:py-16 bg-[#F8F9FA]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap gap-3">
          {moods.map((m) => (
            <button key={m.label} onClick={() => onSelect(m.video)} className="group inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 shadow-sm hover:shadow transition-all">
              <span>{m.label}</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-2xl">
            ...
            <div className="flex items-center justify-between p-4 border-t">
              ...
              <button onClick={() => setOpen(false)} className="rounded-full px-4 py-2 text-sm font-medium text-white" style={{ background: brand.primary }}>Close</button>
            </div>
          </div>
        </div>
      )}
