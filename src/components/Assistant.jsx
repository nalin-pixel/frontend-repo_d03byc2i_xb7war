  ...
  return (
    <>
      <div className="fixed bottom-5 right-5 z-50">
        <button onClick={() => setOpen(true)} className="rounded-full bg-[#6AA9FF] p-4 text-white shadow-lg hover:shadow-xl transition-shadow">
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            <div className="h-80 overflow-y-auto p-4 space-y-3">
              <div className="rounded-lg bg-[#F8F9FA] p-3 text-[11px] text-slate-500">{systemPrompt}</div>
              {messages.map((m, idx) => (
                <div key={idx} className={`${m.role === 'assistant' ? 'bg-[#F8F9FA]' : 'bg-[#E3DCC2]'} rounded-2xl px-3 py-2 text-slate-800 text-sm max-w-[85%] ${m.role === 'assistant' ? '' : 'ml-auto'}`}>
                  {m.content}
                </div>
              ))}
              <div ref={endRef} />
            ...
            <div className="p-3 border-t">
              <div className="flex items-center gap-2">
                <input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Write a message..." className="flex-1 rounded-full border border-slate-200 px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-[#6AA9FF]" />
                <button onClick={onSend} className="rounded-full bg-[#6AA9FF] p-2 text-white shadow">
